...
export function validateCreateCapsuleInput(input: Record<string, unknown>): CreateCapsuleValidationResult {
  ...
  if (title.length > 100) {
    return { error: '标题长度不能超过 100 个字符' }
  }
  if (!content) {
    return { error: '内容不能为空' }
  }
  if (!creator) {
    return { error: '发布者不能为空' }
  }
  if (creator.length > 30) {
    return { error: '发布者长度不能超过 30 个字符' }
  }
  if (!openAt) {
    return { error: '开启时间不能为空' }
  }

  const openDate = new Date(openAt)
  if (Number.isNaN(openDate.getTime())) {
    return { error: '开启时间格式无效' }
  }
  if (openDate.getTime() <= Date.now()) {
    return { error: '开启时间必须晚于当前时间' }
  }
...
export function normalizeCapsuleCode(value: string) {
  return safeDecodeURIComponent(value).toUpperCase()
}
