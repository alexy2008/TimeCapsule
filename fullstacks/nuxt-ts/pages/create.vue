<template>
  <section v-if="created" id="view-created" class="view active">
    <div class="success-container cyber-glass text-center">
      ...
      <button class="btn btn-outline mt-6" @click="navigateTo('/')">返回首页</button>
    </div>
  </section>
...
useSeoMeta({
  title: '创建胶囊 - 时间胶囊-Nuxt',
  description: '通过 Nuxt 全栈实现创建并封存时间胶囊',
})
...
async function confirmCreate() {
  ...
  try {
    created.value = await create(pendingForm.value)
  } catch {
