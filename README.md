...
HelloTime 是一个多技术栈对比学习项目。通过同一个业务场景（时间胶囊），展示 6 个后端框架、4 个前端框架、3 套全栈样板以及 2 套桌面端实现。项目重点不在业务逻辑本身，而在于展示如何使用不同技术栈实现相同的 API 契约和设计系统，帮助开发者快速对比框架差异、学习最佳实践、理解架构设计。
...
前端开发环境通常通过 `http://localhost:8080` 访问后端，可使用 `scripts/switch-backend.sh` 动态切换 `8080 -> 18xxx` 的映射。`fullstacks/next-ts`、`fullstacks/nuxt-ts` 和 `fullstacks/spring-boot-mvc` 为独立全栈实现，不依赖该映射。
