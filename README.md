...
HelloTime 是一个多技术栈对比学习项目。通过同一个业务场景（时间胶囊），展示 10 个后端框架、5 个前端框架、3 套全栈样板以及 3 套桌面端实现。项目重点不在业务逻辑本身，而在于展示如何使用不同技术栈实现相同的 API 契约和设计系统，帮助开发者快速对比框架差异、学习最佳实践、理解架构设计。
...
| **SolidJS** | `frontends/solid-ts/` | Vite 7 | 5180 |
...
| **Ktor** | `backends/ktor/` | Kotlin + JVM | 18090 |
| **Vapor** | `backends/vapor/` | Swift 6.2+ | 18060 |
| **Axum** | `backends/axum/` | Rust 1.94+ | 18070 |
| **Drogon** | `backends/drogon/` | C++20 + CMake | 18080 |

前端开发环境通常通过 `http://localhost:8080` 访问后端，可使用 `scripts/switch-backend.sh`（Unix）或 `scripts/switch-backend.ps1`（Windows）动态切换 `8080 -> 18xxx` 的映射。`frontends/solid-ts` 也遵循这一套前后端分离模型；`fullstacks/next-ts`、`fullstacks/nuxt-ts` 和 `fullstacks/spring-boot-mvc` 为独立全栈实现，不依赖该映射。
...
- [Solid 前端说明](frontends/solid-ts/README.md)
...
│   ├── solid-ts/            # SolidJS + TypeScript + Vite
│   └── ui-prototype/        # 前端原型设计
...
│   ├── ktor/                # Ktor + Kotlin + JDBC SQLite
│   ├── vapor/               # Vapor + Swift + Fluent SQLite
│   ├── axum/                # Axum + Rust + SQLx SQLite
│   └── drogon/              # Drogon + C++20 + SQLite3
├── verification/            # 共享验证脚本与验证矩阵
└── scripts/                 # 开发/构建/测试脚本
    ├── dev.sh               # 启动所有开发服务
    ├── dev-manager.py       # Web 优先的服务管理工具
...
.\scripts\switch-backend.ps1 ktor
...
- 启动 Vue 3 / Angular / Svelte / React / Solid 前端
