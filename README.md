...
HelloTime 是一个 RealWorld 风格的多实现技术展示项目。仓库中的前后端实现彼此独立，但遵循同一套 API 契约、设计系统和验证标准，从而支持任意前端与任意后端自由组合；同时也包含完整的全栈实现样板。
...
前端开发环境通常通过 `http://localhost:8080` 访问后端，可使用 [scripts/switch-backend.sh](/Users/alex/AiWork/HelloTimeByClaude/scripts/switch-backend.sh) 动态切换 `8080 -> 18xxx` 的映射。`fullstacks/next-ts`、`fullstacks/nuxt-ts` 和 `fullstacks/spring-boot-mvc` 为独立全栈实现，不依赖该映射。
...
- [全栈实现对比分析](docs/fullstack-comparison.md)
